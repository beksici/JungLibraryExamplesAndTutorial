...
public class Step2Sample2 {
//rastgele layout değil düzenli layout örneği
	public static void main(String[] args) {
    	Dimension viewArea = new Dimension(300, 300);
        Graph<MyNode,MyEdge> graph = createGraph();
        //FRlayout sayesinde daha düzenli yerleşiyor
        /*FRLayout'a ek olarak, aşağıdaki algoritmalar da kullanılabilir:

KKLayout: Kamada ve Kawai kuvvet odaklı algoritmalar
SpringLayout: Yay gömülü model (Peter Eades'in modelinden farklı görünüyor)
CircleLayout: Circumferential (düğümler çevrenin etrafında eşit aralıklarla yerleştirilir)*/
        //Layout<MyNode,MyEdge> layout = new FRLayout<MyNode,MyEdge>(graph, viewArea);
       // Layout<MyNode,MyEdge> layout = new KKLayout<>(graph);
       // Layout<MyNode,MyEdge> layout = new SpringLayout(graph);
       //Layout<MyNode,MyEdge> layout = new CircleLayout(graph);
        Layout<MyNode,MyEdge> layout = new FRLayout<MyNode,MyEdge>(graph, viewArea);
        BasicVisualizationServer<MyNode,MyEdge> panel = new BasicVisualizationServer<MyNode,MyEdge>(layout, viewArea);

        JFrame frame = new JFrame("Graph View: F&R Layout");
